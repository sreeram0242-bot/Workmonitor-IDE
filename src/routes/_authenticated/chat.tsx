...
import {
  Plus, Send, Users, MessageSquare, ArrowLeft, Paperclip, X, FileText,
  Image as ImageIcon, Film, Search, Sparkles, Smile, MoreVertical, Pencil,
  Trash2, LogOut, UserPlus, Settings2, Check, Reply, Pin, PinOff, Forward,
  Star, Mic, Square as StopIcon, CheckCheck, Link as LinkIcon, Bell, BellOff,
  Wand2,
} from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/chat")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Real-time direct messages, group chat, and file sharing for the C-Enterprises team." },
      { property: "og:title", content: "Team Chat — C-Enterprises WorkMonitor" },
      { property: "og:description", content: "Collaborate with real-time messages, groups, photos, videos, and files." },
...
function Avatar({ name, group, admin, size = 40, online, url }: { name: string; group?: boolean; admin?: boolean; size?: number; online?: boolean; url?: string | null }) {
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <div className={`flex h-full w-full items-center justify-center overflow-hidden rounded-full ${bgClass} shadow-sm ring-2 ring-background`}>
        {hasImage ? (
...
function ChatPage() {
  ...
  async function reloadConversations() {
    ...
    await Promise.all(
      convs.map(async (c) => { memberMap[c.id] = await fetchConversationMembers(c.id); }),
    );
  ...
  useEffect(() => {
    ...
    if (!user) return;
    const ch = supabase
      .channel(`chat-conversations-${user.id}-${Math.random().toString(36).slice(2, 8)}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "conversations" }, () => reloadConversations())
      .on("postgres_changes", { event: "*", schema: "public", table: "conversation_members" }, () => reloadConversations())
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        const m = payload.new as Message;
        if (m.sender_id === user.id) return;
        if (activeIdRef.current === m.conversation_id) {
          markConversationRead(m.conversation_id, user.id).then(() => {
            setLastRead((prev) => ({ ...prev, [m.conversation_id]: new Date().toISOString() }));
          });
          return;
        }
  ...
  useEffect(() => {
    if (!activeId || !user) {
      setMessages([]); setOnlineUsers(new Set()); setTypingUsers(new Set());
      setHasMoreOlder(false); setReactions([]); setPinned([]); setReplyTo(null);
      setInChatSearch(false); setInChatQuery(""); setReadByMap({});
      return;
    }
    ...
    const dbCh = supabase
      .channel(`msgs-${activeId}-${Math.random().toString(36).slice(2, 8)}`)
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "messages", filter: `conversation_id=eq.${activeId}` },
        (payload) => {
          ...
          if (nm.pinned_at) setPinned((prev) => prev.some((p) => p.id === nm.id) ? prev.map((p) => p.id === nm.id ? nm : p) : [nm, ...prev]);
          else setPinned((prev) => prev.filter((p) => p.id !== nm.id));
        })
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "message_reactions" },
        () => { fetchReactions(activeId).then(setReactions); })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "conversation_members", filter: `conversation_id=eq.${activeId}` },
        () => { fetchLastReadByMembers(activeId).then(setReadByMap); })
      .subscribe();

    const presenceCh = supabase.channel(`presence-${activeId}`, {
    ...
    presenceCh
      ...
      .on("broadcast", { event: "typing" }, (payload) => {
        ...
        setTypingUsers((prev) => {
          const next = new Set(prev); next.add(uid); return next;
        });
        window.setTimeout(() => {
      ...
      .subscribe(async (status) => {
        if (status === "SUBSCRIBED" && !(profile as any)?.presence_hidden) {
  ...
  function broadcastTyping() {
    ...
    supabase.channel(`presence-${activeId}`).send({
    ...
    try {
      ...
      if (others.length > 0) {
        const { data: prefs } = await supabase
          .from("profiles")
          .select("id, notify_messages")
          .in("id", others);
        const allowSet = new Set((prefs ?? []).filter((p: any) => p.notify_messages !== false).map((p: any) => p.id));
        for (const uid of others) {
          if (mentionIds.includes(uid)) {
            // Always deliver @mentions
            notifications.push({ user_id: uid, type: "mention", message: `${senderName} mentioned you`, link: "/chat" });
          } else if (allowSet.has(uid)) {
            notifications.push({ user_id: uid, type: "message", message: `New message from ${senderName}`, link: "/chat" });
          }
        }
      }
  ...
  async function handleReactionToggle(messageId: string, emoji: string) {
    ...
    if (has) {
      ...
      setReactions((prev) => [...prev, { id: `tmp-${Math.random()}`, message_id: messageId, user_id: user.id, emoji }]);
    }
    try { await toggleReaction(messageId, user.id, emoji, has); }
    catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Reaction failed"); }
  }

  async function handleTogglePin(m: Message) {
    if (!user) return;
    try { await togglePinMessage(m.id, user.id, !m.pinned_at); }
    catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Pin failed"); }
  }
  ...
  async function handleToggleStar(id: string) {
    ...
    setStarredIds(next);
    try { await toggleStar(id, user.id, has); }
    catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Star failed"); setStarredIds(starredIds); }
  }

  async function handleForwardTo(targetConvId: string) {
    if (!user || !forwardMessage) return;
    try {
      await sendMessage(targetConvId, user.id, forwardMessage.content, forwardMessage.attachments ?? [], {
        forwarded_from: forwardMessage.id,
      });
      toast.success("Message forwarded");
      setForwardMessage(null);
      if (targetConvId !== activeId) setActiveId(targetConvId);
    } catch (e: unknown) { toast.error(e instanceof Error ? e.message : "Forward failed"); }
  }
  ...
  function cancelRecording() {
    ...
    if (mr) { recordChunksRef.current = []; mr.onstop = () => { mr.stream.getTracks().forEach((t) => t.stop()); }; mr.stop(); }
    if (recordTimerRef.current) { window.clearInterval(recordTimerRef.current); recordTimerRef.current = null; }
  ...
  async function handleDelete(id: string) {
    ...
    try { await deleteMessage(id); } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : "Failed to delete");
    }
  }

  async function handleDeleteForMe(id: string) {
    try { await deleteMessageForMe(id); } catch (e: unknown) {
  ...
  const typingNames = Array.from(typingUsers).map((id) => teamById.get(id)?.full_name?.split(" ")[0]).filter(Boolean) as string[];
  ...
  const visibleMessages = (inQ ? messages.filter((m) => m.content?.toLowerCase().includes(inQ)) : messages)
    .filter((m) => !user || !(m as any).deleted_for || !(((m as any).deleted_for as string[]) ?? []).includes(user.id));
  ...
  return (
    <>
      <div className="grid h-[100dvh] grid-cols-1 gap-0 overflow-hidden bg-card/60 md:grid-cols-[300px_1fr]">
        {/* Sidebar */}
        <div className={`flex flex-col overflow-hidden border-r border-border/60 bg-background/70 ${activeId ? "hidden md:flex" : "flex"}`}>
          ...
          <ScrollArea className="flex-1">
            <div className="p-2">
              <div className="px-2 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Groups & DMs</div>
              ...
              {filteredConvs.map((c) => {
                ...
                return (
                  ...
                  >
                    <Avatar name={label} group={c.is_group} admin={conversationAdmin(c)} size={36} online={!!(otherId && onlineUsers.has(otherId))} url={otherId ? teamById.get(otherId)?.avatar_url : null} />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        <span className={`truncate ${unreadCount > 0 ? "font-semibold" : "font-medium"}`}>{label}</span>
                        {mutes[c.id] && <BellOff className="h-3 w-3 shrink-0 text-muted-foreground" />}
                        {c.is_group && <Badge variant="outline" className="ml-auto text-[9px]">group</Badge>}
                      </div>
                      <div className="truncate text-xs text-muted-foreground">
                        {c.is_group ? `${(convMembers[c.id] ?? []).length} members` : "Direct message"}
                      </div>
                    </div>
                    {unreadCount > 0 && (
              ...
              <div className="px-2 pb-1 pt-4 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Start a DM</div>
              {filteredTeam.map((m) => {
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      <Avatar name={m.full_name} admin={m.role === "admin"} size={36} url={m.avatar_url} online={onlineUsers.has(m.id)} />
                      {dmUnread > 0 && (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5">
                        <span className={`truncate ${dmUnread > 0 ? "font-semibold" : "font-medium"}`}>{m.full_name}</span>
                        {m.badge && <BadgePill label={m.badge} size="xs" />}
                        {m.role === "admin" && <Badge variant="outline" className="ml-auto text-[9px]">admin</Badge>}
                      </div>
        ...
        {/* Thread */}
        <div className={`relative flex min-w-0 flex-col overflow-hidden rounded-xl border border-border/60 bg-background/70 ${activeId ? "flex" : "hidden md:flex"}`}>
          {activeConv ? (
            <>
              <div className="flex items-center gap-3 border-b border-border/60 bg-white/70 px-4 py-3 text-foreground">
                <Button size="icon" variant="ghost" onClick={() => setActiveId(null)} aria-label="Back to conversations">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="truncate text-sm font-semibold">{conversationLabel(activeConv)}</span>
                    {conversationBadge(activeConv) && <BadgePill label={conversationBadge(activeConv)!} size="xs" />}
                  </div>
                ...
                {activeConv.is_group && user && (
                  <Button size="icon" variant="ghost" onClick={() => setManageOpen(true)} aria-label="Manage group">
                ...
                {user && (
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try { await toggleMuteConversation(activeConv.id, next); }
                      catch (e: unknown) {
                        setMutes((m) => ({ ...m, [activeConv.id]: !next }));
                  ...
                  >
                    {mutes[activeConv.id] ? <BellOff className="h-4 w-4 text-amber-600" /> : <Bell className="h-4 w-4" />}
                  </Button>
                )}
                <Button size="icon" variant="ghost" onClick={() => setInChatSearch((v) => !v)} aria-label="Search in chat" className="hidden sm:inline-flex">
                  <Search className="h-4 w-4" />
                </Button>
                <Button size="icon" variant="ghost" onClick={openStarredPanel} aria-label="Starred messages" className="hidden sm:inline-flex">
                ...
                <div className="border-b border-border/60 bg-amber-50/70 px-4 py-2">
                  ...
                  <div className="mt-1 flex gap-2 overflow-x-auto">
                    {pinned.slice(0, 5).map((p) => (
                      ...
                      >
                        <span className="font-medium text-amber-700">{teamById.get(p.sender_id)?.full_name?.split(" ")[0] ?? "Msg"}:</span>{" "}
                    ...
                    <div key={m.id} id={`msg-${m.id}`} className={`group/msg flex items-end gap-2 ${mine ? "justify-end" : "justify-start"} rounded-lg transition-shadow`}>
                      ...
                      >
                        ...
                        {m.pinned_at && !isDeleted && (
                          <div className={`mb-1 flex items-center gap-1 text-[10px] font-semibold ${mine ? "text-white/80" : "text-amber-600"}`}>
                            <Pin className="h-3 w-3" /> Pinned
                          </div>
                        )}
                        {m.forwarded_from && !isDeleted && (
                          <div className={`mb-1 flex items-center gap-1 text-[10px] italic ${mine ? "text-white/80" : "text-muted-foreground"}`}>
                        ...
                        {parent && (
                          ...
                          >
                            <span className="font-semibold">{teamById.get(parent.sender_id)?.full_name?.split(" ")[0] ?? "Reply"}: </span>
                            {parent.deleted_at ? "(deleted)" : parseEffect(parent.content).text || "(attachment)"}
                          </button>
                        )}

                        {isDeleted ? (
                          <div className="whitespace-pre-wrap break-words">This message was deleted</div>
                        ) : isEditing ? (
                          <div className="space-y-1">
                            <Input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter") { e.preventDefault(); handleEditSave(m.id); }
                            ...
                            <div className="flex gap-1">
                              <Button size="sm" variant="secondary" className="h-6 px-2 text-xs" onClick={() => handleEditSave(m.id)}>Save</Button>
                          ...
                          <>
                            {atts.length > 0 && (
                              <div className={`mb-1 grid gap-1.5 ${atts.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
                                {atts.map((a, i) => <AttachmentView key={i} att={a} mine={mine} onOpenImage={setLightboxUrl} />)}
                              </div>
                            )}
                            {fxText && (
                        ...
                        <div className={`mt-1 flex items-center gap-1 text-[10px] ${mine ? "text-white/70" : "text-muted-foreground"}`}>
                          <span title={new Date(m.created_at).toLocaleString()}>{formatRelative(m.created_at)}</span>
                        ...
                        {!isDeleted && !isEditing && (
                          <div className={`absolute -top-3 ${mine ? "right-1" : "left-1"} flex items-center gap-0.5 rounded-full bg-white/95 px-0.5 shadow ring-1 ring-black/5 opacity-0 transition-opacity group-hover/msg:opacity-100`}>
                            <Popover>
                              ...
                              <PopoverContent side="top" align={mine ? "end" : "start"} className="w-auto p-1">
                            ...
                            <button className="rounded-full p-1 text-slate-700 hover:bg-accent" aria-label="Reply" onClick={() => setReplyTo(m)}>
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align={mine ? "end" : "start"} className="w-44">
                                ...
                                <DropdownMenuItem onSelect={() => handleToggleStar(m.id)}>
                                  <Star className={`mr-2 h-3.5 w-3.5 ${isStarred ? "fill-current text-amber-500" : ""}`} /> {isStarred ? "Unstar" : "Star"}
                                </DropdownMenuItem>
                                <DropdownMenuItem onSelect={() => handleTogglePin(m)}>
                                  {m.pinned_at ? <><PinOff className="mr-2 h-3.5 w-3.5" /> Unpin</> : <><Pin className="mr-2 h-3.5 w-3.5" /> Pin</>}
                                </DropdownMenuItem>
                                <DropdownMenuItem onSelect={() => setForwardMessage(m)}>
                                  <Forward className="mr-2 h-3.5 w-3.5" /> Forward
                                </DropdownMenuItem>
                                {fxText && (
                                  <DropdownMenuItem onSelect={() => { navigator.clipboard.writeText(fxText); toast.success("Copied"); }}>
                                ...
                                {mine && fxText && (
                                  <>
                                    ...
                                    <DropdownMenuItem onSelect={() => { setEditingId(m.id); setEditingText(fxText); }}>
                                ...
                                {mine && (
                                  <DropdownMenuItem className="text-destructive focus:text-destructive" onSelect={() => handleDelete(m.id)}>
                ...
                {typingNames.length > 0 && (
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <span className="inline-flex gap-0.5">
                      ...
                      <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-brand-accent" style={{ animationDelay: "120ms" }} />
                      <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-brand-accent" style={{ animationDelay: "240ms" }} />
                    </span>
                    <span>{typingNames.slice(0, 2).join(", ")}{typingNames.length > 2 ? ` +${typingNames.length - 2}` : ""} typing…</span>
                  </div>
                )}
              </div>
              {replyTo && (
                <div className="flex items-start gap-2 border-t border-border/60 bg-brand-accent/5 px-3 py-2">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="truncate text-xs text-muted-foreground">
                      {replyTo.deleted_at ? "(deleted)" : parseEffect(replyTo.content).text || "(attachment)"}
                    </div>
                  </div>
                  <button type="button" onClick={() => setReplyTo(null)} className="rounded-full p-1 hover:bg-accent" aria-label="Cancel reply">
              ...
              {recording && (
                <div className="flex items-center gap-3 border-t border-border/60 bg-red-50 px-3 py-2 text-sm">
                  ...
                  <span className="font-medium text-red-700">Recording {fmtSec(recordSeconds)}</span>
              ...
              {pendingFiles.length > 0 && (
                <div className="flex flex-wrap gap-2 border-t border-border/60 bg-muted/40 p-2">
                  {pendingFiles.map((f, i) => {
                    ...
                    return (
                      <div key={i} className="group relative flex items-center gap-2 rounded-md border border-border bg-background px-2 py-1.5 text-xs shadow-sm">
                        {isImg ? <ImageIcon className="h-3.5 w-3.5 text-brand-accent" /> : isVid ? <Film className="h-3.5 w-3.5 text-brand-accent" /> : isAud ? <Mic className="h-3.5 w-3.5 text-brand-accent" /> : <FileText className="h-3.5 w-3.5 text-brand-accent" />}
              ...
              >
                ...
                <EmojiPicker onPick={(emoji) => setInput((prev) => prev + emoji)} disabled={uploading} />
                {profile?.badge === "Developer" && (
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {pendingEffect
                          ? <span className="text-base leading-none">{MESSAGE_EFFECTS.find((e) => e.id === pendingEffect)?.icon}</span>
                          : <Wand2 className="h-4 w-4" />}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent side="top" align="start" className="w-64 p-2">
                      <div className="mb-1 flex items-center justify-between px-1">
                        <div className="text-xs font-semibold text-slate-700">Developer effects</div>
                        {pendingEffect && (
                          <button type="button" onClick={() => setPendingEffect(null)} className="text-[10px] text-muted-foreground hover:text-destructive">Clear</button>
                        )}
                      </div>
            ...
            <div className="flex flex-1 items-center justify-center p-8 text-center">
              <div className="max-w-xs">
                ...
                <div className="font-display text-lg font-semibold">Your conversations live here</div>
                <p className="mt-1 text-sm text-muted-foreground">Select a chat from the left or start a new group to get talking with your team.</p>
              </div>
            </div>
          )}
        </div>
      ...
      {forwardMessage && user && (
        <Dialog open={!!forwardMessage} onOpenChange={(v) => { if (!v) setForwardMessage(null); }}>
          <DialogContent>
            ...
            <div className="max-h-72 space-y-1 overflow-y-auto">
              {conversations.map((c) => (
                ...
                >
                  <Avatar name={conversationLabel(c)} group={c.is_group} admin={conversationAdmin(c)} size={28} url={otherUserId(c) ? teamById.get(otherUserId(c) ?? "")?.avatar_url : null} />
                  <span className="flex-1 truncate">{conversationLabel(c)}</span>
                  {c.is_group && <Badge variant="outline" className="text-[9px]">group</Badge>}
                </button>
              ))}
            </div>
          </DialogContent>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="max-h-96 space-y-2 overflow-y-auto">
            {starredMessages.length === 0 && (
              <div className="py-8 text-center text-sm text-muted-foreground">No starred messages yet.</div>
            )}
            {starredMessages.map((m) => (
              <div key={m.id} className="rounded-md border border-border bg-muted/30 p-2 text-sm">
                <div className="mb-1 flex items-center justify-between text-[11px] text-muted-foreground">
                  <span className="font-semibold text-slate-700">{teamById.get(m.sender_id)?.full_name ?? "Member"}</span>
                  <span title={new Date(m.created_at).toLocaleString()}>{formatRelative(m.created_at)}</span>
                </div>
                <div className="whitespace-pre-wrap break-words">{m.deleted_at ? <em>(deleted)</em> : m.content || <em>(attachment)</em>}</div>
                <div className="mt-1 flex justify-end gap-2">
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setTimeout(() => document.getElementById(`msg-${m.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 400);
                    }}
...
function renderContentWithMentions(text: string, team: TeamMember[], mine: boolean) {
  ...
  while ((match = re.exec(text))) {
    ...
    if (hit) {
      parts.push(
        <span key={`m-${key++}`} className={`rounded px-1 font-semibold ${mine ? "bg-white/25 text-white" : "bg-brand-accent/15 text-brand-accent"}`}>@{hit.full_name.split(" ")[0]}</span>,
      );
    } else {
      parts.push(match[0]);
    }
  ...
  return (
    <a
      ...
      className={`mt-1.5 flex items-center gap-2 rounded-md border px-2 py-1.5 text-[11px] transition-colors ${
        mine ? "border-white/30 bg-white/10 text-white/90 hover:bg-white/20" : "border-border bg-muted/40 text-slate-700 hover:bg-muted"
      }`}
    >
...
function NewGroupDialog({
  open, onOpenChange, team, onCreate,
}: {
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { onOpenChange(v); if (!v) { setName(""); setSelected(new Set()); } }}>
      <DialogTrigger asChild>
        <Button size="sm" className="h-8 rounded-full border border-white/30 bg-white/15 px-3 text-white hover:bg-white/25">
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Marketing squad" />
          </div>
          <div className="space-y-2">
            <Label>Members</Label>
            <div className="max-h-56 space-y-1 overflow-y-auto rounded-md border border-border p-2">
              {team.map((m) => (
                <label key={m.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-accent">
                  ...
                  />
                  <span className="flex-1">{m.full_name} <span className="text-xs text-muted-foreground">— {m.position}</span></span>
        ...
        <DialogFooter>
          <Button
            ...
            onClick={async () => {
              setBusy(true);
              try { await onCreate(name.trim(), Array.from(selected)); }
...
function ManageGroupDialog({
  open, onOpenChange, conversation, memberIds, team, currentUserId, onChanged, onLeave,
}: {
  open: boolean;
  onOpenChange: (v: boolean) => void;
  ...
  useEffect(() => {
    if (open) { setName(conversation.name ?? ""); setToAdd(new Set()); }
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input value={name} onChange={(e) => setName(e.target.value)} disabled={!isCreator || busy} />
              {isCreator && (
                <Button onClick={saveName} disabled={busy || !name.trim() || name.trim() === (conversation.name ?? "")}>
                  <Check className="mr-1 h-4 w-4" /> Save
                </Button>
              )}
            </div>
            {!isCreator && <p className="text-xs text-muted-foreground">Only the creator can rename this group.</p>}
          </div>
              ...
              {currentMembers.map((m) => (
                <div key={m.id} className="flex items-center gap-2 rounded px-2 py-1.5 text-sm">
                  <span className="flex-1">
                    {m.full_name} <span className="text-xs text-muted-foreground">— {m.position}</span>
                    {m.id === conversation.created_by && <Badge variant="outline" className="ml-2 text-[9px]">creator</Badge>}
                  </span>
                  {isCreator && m.id !== currentUserId && (
                    <Button size="icon" variant="ghost" onClick={() => removeMember(m.id)} disabled={busy} aria-label="Remove member">
                      <X className="h-3.5 w-3.5" />
                    </Button>
                  )}
                </div>
...
function AttachmentView({ att, mine, onOpenImage }: { att: ChatAttachment; mine: boolean; onOpenImage?: (url: string) => void }) {
  const [url, setUrl] = useState<string>(att.url);
  useEffect(() => {
    let cancelled = false;
    supabase.storage.from("chat-attachments").createSignedUrl(att.path, 60 * 60 * 24).then(({ data }) => {
  ...
  if (att.kind === "image") {
    return (
      <button type="button" onClick={() => onOpenImage?.(url)} className="block overflow-hidden rounded-lg">
        <img src={url} alt={att.name} className="max-h-64 w-full cursor-zoom-in object-cover transition-transform hover:scale-[1.01]" loading="lazy" />
      </button>
    );
  }
  if (att.kind === "video") {
  ...
  if (att.kind === "audio") {
    return (
      <div className={`flex items-center gap-2 rounded-lg px-2.5 py-2 ${mine ? "bg-white/10" : "bg-muted/40"}`}>
  ...
  const isPdf = att.kind === "file" && (att.name.toLowerCase().endsWith(".pdf") || (att as unknown as { mime?: string }).mime === "application/pdf");
  if (isPdf) {
    return (
      <div className={`overflow-hidden rounded-lg border ${mine ? "border-white/30 bg-white/10" : "border-border bg-background"}`}>
        <object data={url} type="application/pdf" className="h-64 w-full">
          <a href={url} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-2 text-xs underline">
            <FileText className="h-4 w-4" /> {att.name}
          </a>
        </object>
        <a href={url} target="_blank" rel="noreferrer" download={att.name} className={`flex items-center gap-2 border-t px-2.5 py-1.5 text-xs ${mine ? "border-white/20" : "border-border"}`}>
